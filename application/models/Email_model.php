<?php
    ...
    class Email_model extends CI_Model {
        ...
        function newsletter($title = '', $text = '', $email = '', $from = '') {
            $from_name = $this->db->get_where('general_settings', array('type' => 'system_name'))->row()->value;
            $protocol = $this->db->get_where('general_settings', array('type' => 'mail_status'))->row()->value;
            $this->do_email($protocol,$from, $from_name, $email, $title, $text);
        }
        ...
        function profile_report($from = '', $from_name ='', $reported_person = '',$from_id = '')
        {
            ...
            $send_mail = $this->do_email($protocol,$from, $from_name, $to, $sub, $email_body,$cc);
            return $send_mail;
        ...
        function accept_mail_send($member_id,$senders_id,$sender_email = '', $sender_name = '', $to = '', $sub = '')
        {
            ...
            $msg_body = "<table width='400' border='0' style='font-family:Arial, Helvetica, sans-serif; font-size:12px; border:1px solid #999; padding:10px; '>
                ...
                $send_mail = $this->do_accept_email($protocol,$from, $from_name, $to, $sub, $msg_body);
                return $send_mail;
            } else {
                return false;
            }
        ...
        function contactUs_mail_send($from = '', $from_name ='', $subject = '',$msg = '')
        {
            ...
            $sub = $from.'('. $subject .')';
            $send_mail = $this->do_email($protocol,$to, $to_name, $to, $sub, $msg);
            return $send_mail;
        ...
        function do_email($protocol='',$from = '', $from_name = '', $to = '', $sub = '', $msg = '',$cc = '') {
            ...
            if($protocol == 'smtp'){
                ...
                $config['smtp_host'] = $this->db->get_where('general_settings', array('type' => 'smtp_host'))->row()->value;//need to change; Mandrill: smtp.mandrillapp.com
                // $config['smtp_port'] = 587;
                $config['smtp_port'] = $this->db->get_where('general_settings', array('type' => 'smtp_port'))->row()->value;//need to change; Mandrill: 587                
                // $config['smtp_user'] = 'clickmarry.in@gmail.com';
                $config['smtp_user'] = $from;//need to change
                // $config['smtp_pass'] = 'hblrfudgnnfjrcfv';
                $config['smtp_pass'] = $this->db->get_where('general_settings', array('type' => 'smtp_pass'))->row()->value;//need to change
            ...
            $this->email->from('helpdesk@clickmarry.in', $from_name);
            $this->email->to($to);
            $this->email->bcc($cc);
        ...
        function do_accept_email($protocol='',$from = '', $from_name = '', $to = '', $sub = '', $msg = '') {
            // print_r($msg);
            $this->load->library('email');
            if($protocol == 'smtp'){
