...
class Admin extends CI_Controller {
	...
	{		
		if ($this->admin_permission() == FALSE) {
			...
			elseif ($this->session->flashdata('alert')=="otp_validated"){
				$page_data['success_alert'] = translate("you_have_successfully_validate_OTP_this_member!");
			}
			if ($para2=="list_data") {
		        ...
		        if(!empty($members))
		        {
		            ...
		            foreach ($members as $member)
		            {
						...
						$nestedData['member_since'] = date('d/m/Y', strtotime($member->member_since));
						$nestedData['otp_validated']=$member->is_otp_validated != 1 ?'No':'Yes';
		                $nestedData['member_status'] = $acnt_status_button;

						$btnOptionsAdmin = "<a href='".base_url()."admin/members/".$para1."/view_member/".$member->member_id."' id='demo-dt-view-btn' class='btn btn-primary btn-xs add-tooltip' data-toggle='tooltip' data-placement='top' title='".translate('view_profile')."' target=_blank><i class='fa fa-eye'></i></a>
						<a href='#' id='demo-dt-delete-btn' data-target='#package_modal' data-toggle='modal' class='btn btn-info btn-xs add-tooltip' data-toggle='tooltip' data-placement='top' title='".translate('packages')."' onclick='view_package(".$member->member_id.")'><i class='fa fa-object-ungroup'></i></a> ".$block_button." <button data-target='#otp_modal' data-toggle='modal' class='btn btn-success btn-xs add-tooltip' data-toggle='tooltip' data-placement='top' title='".translate('otp_validate')."' onclick='otp_validate(".$member->member_id.")'><i class='fa fa-mobile'></i></button> <button data-target='#delete_modal' data-toggle='modal' class='btn btn-danger btn-xs add-tooltip' data-toggle='tooltip' data-placement='top' title='".translate('delete_member')."' onclick='delete_member(".$member->member_id.")'><i class='fa fa-trash'></i></button>";
		                $btnOptionsNonAdmin = "<a href='".base_url()."admin/members/".$para1."/view_member/".$member->member_id."' id='demo-dt-view-btn' class='btn btn-primary btn-xs add-tooltip' data-toggle='tooltip' data-placement='top' title='".translate('view_profile')."' target=_blank><i class='fa fa-eye'></i></a> <button data-target='#otp_modal' data-toggle='modal' class='btn btn-success btn-xs add-tooltip' data-toggle='tooltip' data-placement='top' title='".translate('otp_validate')."' onclick='otp_validate(".$member->member_id.")'><i class='fa fa-mobile'></i></button> ";
		               	
						$btmOption = $loginedType==1?$btnOptionsAdmin:$btnOptionsNonAdmin;

						$nestedData['options'] = $btmOption;
						// $nestedData['options'] = "<a href='".base_url()."admin/members/".$para1."/view_member/".$member->member_id."' id='demo-dt-view-btn' class='btn btn-primary btn-xs add-tooltip' data-toggle='tooltip' data-placement='top' title='".translate('view_profile')."' target=_blank><i class='fa fa-eye'></i></a>
						// 	<a href='#' id='demo-dt-delete-btn' data-target='#package_modal' data-toggle='modal' class='btn btn-info btn-xs add-tooltip' data-toggle='tooltip' data-placement='top' title='".translate('packages')."' onclick='view_package(".$member->member_id.")'><i class='fa fa-object-ungroup'></i></a> ".$block_button." <button data-target='#otp_modal' data-toggle='modal' class='btn btn-success btn-xs add-tooltip' data-toggle='tooltip' data-placement='top' title='".translate('otp_validate')."' onclick='otp_validate(".$member->member_id.")'><i class='fa fa-mobile'></i></button> <button data-target='#delete_modal' data-toggle='modal' class='btn btn-danger btn-xs add-tooltip' data-toggle='tooltip' data-placement='top' title='".translate('delete_member')."' onclick='delete_member(".$member->member_id.")'><i class='fa fa-trash'></i></button>";
						$data[] = $nestedData;						
			...
			elseif ($para1=="upgrade_member_package") {
				...
				$prev_contacts = $this->db->get_where('member', array('member_id' => $up_member_id))->row()->contacts;
                if ($plan_id == '1') {
                	$data['membership'] = 1;
                } else {
                	$data['membership'] = 2;
				...
				$data['contacts'] = $prev_contacts + $this->db->get_where('plan', array('plan_id' => $plan_id))->row()->view_contacts;
				// for expire date of plan Mod:Dt:30/01/2021
				$data['exp_date']=strtotime("+".$this->db->get_where('plan', array('plan_id' => $plan_id))->row()->validity_days." day");
				//end of expire date of plan
	...
	function deleted_members($para1="",$para2="",$para3="",$para4="")
	{
		if ($this->admin_permission() == FALSE) {
			...
			elseif ($para1=="") {
				$page_data['page_name'] = "delete_member";
				$page_data['top'] = "members/index.php";
	...
	function member_restore($para1)
	{
		...
		$data['view_contacts'] = $this->db->get_where("deleted_member", array("member_id" => $para1))->row()->view_contacts;
		...
		$data['contacts'] = $this->db->get_where("deleted_member", array("member_id" => $para1))->row()->contacts;
	...
	function member_otp_validated($para1){
		$data['is_otp_validated']='1';
		$this->session->set_flashdata('alert', 'otp_validated');
		$this->db->where('member_id', $para1);
		$this->db->update('member', $data);
		recache();
	}
	...
	function member_delete($para1){
		...
		$data['view_contacts'] = $this->Crud_model->get_type_name_by_id('member', $para1, 'view_contacts');
		...
		$data['contacts'] = $this->Crud_model->get_type_name_by_id('member', $para1, 'contacts');
	...
	function deleted_member_delete($para1){
				$this->db->where('member_id', $para1);
		        $result = $this->db->delete('deleted_member');		       
				if ($result) {
					$this->session->set_flashdata('alert', 'delete');
		        ...
		        if(!empty($rows))
		        {
		            ...
		            {						
		                ...
		                elseif ($row->payment_type == 'Stripe') {
							...
							$nestedData['payment_type'] = "<center><span class='badge badge-success'>".$row->payment_type."</span></center>";
						}
	...
	// export to excel //
	public function exporttoexcel($member_type){
		// file name 
		$memberType = $member_type != 'Free' ? 2 : 1 ;
		// print_r($memberType);exit;
		$filename = $member_type.'_Members_'.date("d-m-Y H:i:s").'.csv'; 
		header("Content-Description: File Transfer"); 
		header("Content-Disposition: attachment; filename=$filename"); 
		header("Content-Type: application/csv; ");
	   	// get data 
		$usersData = $this->Export_model->exportexcel($memberType);
		// file creation 
		$file = fopen('php://output','w');
		$header = array('Member Id','Name','Email','Mobile'); 
		fputcsv($file, $header);
