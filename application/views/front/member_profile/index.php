                    ...
                    <div class="widget">
                        <div class="card z-depth-2-top" id="profile_load">
                            <div class="card-title">
                                ...
                                <!-- <button class="btn pull-right c-base-1" onclick="printDiv(<?= $get_member[0]->member_id ?>)"><i class="fa fa-print"></i> <?= translate("print_profile"); ?></button>                     -->
                                <button class="btn pull-right c-base-1" onclick="do_print('#profile_print')"><i class="fa fa-print"></i> <?= translate("print_profile"); ?></button>                                                      
                            </div>
                          

                            <div class="card-body" style="padding: 1.5rem 0.5rem;" id="profile_print">
                            
                                <div class="col-md-3 mx-auto" style="display:none;" id="member_image">
                                    <?php  $profile_image = $get_member[0]->profile_image;
                                    $images = json_decode($profile_image, true);
                                    $member_name = $get_member[0]->first_name." ".$get_member[0]->last_name;
                                    if (file_exists('uploads/profile_image/'.$images[0]['thumb'])) {?>
                                        <img src="<?=base_url()?>uploads/profile_image/<?=$images[0]['thumb']?>" width="150" height="150">
                                      
                                        <h6 class="c-base-1"><b><?= $member_name?></b></h6>
                               ...
                               <?php
                                ...
                                ?>
                                <?php
                                // $a=  $this->db->get_where('frontend_settings', array('type' => 'hobbies_and_interest'))->row();
                                // print_r($a);
                                // echo("dd");
...
<script>
    function printDiv(id)
    {
        ...
        $('#member_image').show();
        var divToPrint = document.getElementById('profile_print');
        $("#active_print_modal").modal({
            backdrop: 'static',
            keyboard: false
        });
        $("#modal_header_print").html("<?php echo translate('print_profile')?>");
        $("#modal_body_print").html(divToPrint.innerHTML);
        // $("#modal_buttons_print").html("<a href='#' id='' class='btn btn-danger btn-sm btn-shadow' onclick='window.location.reload();' style='width:25%'><?php echo translate('cancel')?></a> <a href='#' id='' class='btn btn-sm btn-base-1 btn-shadow' onclick='return do_print()' style='width:25%'><?php echo translate('print')?></a>");
        
    }
  function do_print(data){
    $('#member_image').show();
    var divToPrint = document.getElementById('profile_print');
    var print_content = $('<div/>').append($(data).clone()).html();
    var newWin = window.open('', 'Print-Window');
    // newWin.document.open();
    // newWin.document.write('<html><body onload="window.print()">' + divToPrint.innerHTML + '</body></html>');
    // Popup($('<div/>').append($(divToPrint).clone()).html());
    newWin.document.write('<html><head>');
    newWin.document.write('<link rel="stylesheet" href="<?= base_url()?>template/front/css/global-style-pink.css" type="text/css" />');
    newWin.document.write('</head><body>');
    newWin.document.write(print_content);    
