    ...
    <div class="sidebar-object mb-0">
        ...
        <div class="profile-connect mt-2">
            <div class="row">
                    ...
                    <?php
                        ...
                        if (!empty($this->session->userdata('member_id'))) {
                            if (in_assoc_array($get_member[0]->member_id, 'id', $interest)) {
                                ...
                                if (in_assoc_array($get_member[0]->member_id, 'id', $interest_on_me)) {
                                    foreach($interest_on_me as $inst){                                               
                                        if($inst['id']==$get_member[0]->member_id){
                    ...
                    <?php if ($interest_text == "Pending") {?>
                        <div class="col-sm-6 pr-1 size-smtr">
                        <a class="btn btn-styled btn-block btn-sm btn-success z-depth-2-bottom" id="accept_<?=$get_member[0]->member_id?>" <?php if ($interest_onclick == 0){?>onclick="return confirm_accept(<?=$get_member[0]->member_id?>)"<?php }?>>
                            <span id="interest_text">
                                <i class="fa fa-heart"></i> <?php echo translate('accept')?>
                            </span>
                        </a>
                        </div>
                        <div class="col-sm-6 pr-1 size-smtl">
                        <a class="btn btn-styled btn-block btn-sm btn-danger z-depth-2-bottom" id="reject_<?=$get_member[0]->member_id?>" <?php if ($interest_onclick == 0){?>onclick="return confirm_reject(<?=$get_member[0]->member_id?>)"<?php }?>>
                            <span id="interest_text">
                                <i class="fa fa-heartbeat"></i> <?php echo translate('decline')?>
                            </span>
                        </a>
                        </div>
                   <?php } else {?>
                    <div class="col-sm-12 size-sm">
                        <a class="<?=$interest_class?>" id="interest_a_<?=$get_member[0]->member_id?>" <?php if ($interest_onclick == 1){?>onclick="return confirm_interest(<?=$get_member[0]->member_id?>)"<?php }?>>
                            <span id="interest_text">
                                <i class="fa fa-heart"></i> <?= $interest_text!=translate('rejected')?$interest_text:translate('declined')?>
                            </span>
                        </a>
                        </div>
                    <?php }?>
            ...
            <div class="row">
                ...
                <div class="col-sm-6 pl-1 size-smtl">
                    ...
                    if ($login_package_info[0]['current_package'] == "Free") {?>
                        ...
                        <a class="<?=$followed_class?>" id="followed_a_<?=$get_member[0]->member_id?>" onclick="return not_a_paid_member(<?=$get_member[0]->member_id?>)">
            ...
            <div class="row mt-2">
                ...
                <div class="col-sm-6 pr-1 size-sml">
                    <?php
                        $view_contacts = $this->Crud_model->get_type_name_by_id('member', $this->session->userdata('member_id'), 'view_contacts');
                        $view_contacts = json_decode($view_contacts, true);
                        if (!empty($this->session->userdata('member_id')))
                        {
                            if (in_assoc_array($get_member[0]->member_id,'id', $view_contacts))
                            {
                                $contact_type = 0;
                                $contact_text =  translate('contact_viewed');
                                $contact_class = "btn btn-styled btn-block btn-sm btn-white z-depth-2-bottom li_active";
                            }
                            else
                            {
                                $contact_type = 1;
                                $contact_text =  translate('contact_view');
                                $contact_class = "btn btn-styled btn-block btn-sm btn-white z-depth-2-bottom";
                            }
                        }
                        else
                        {
                    ...
                    <a class="<?=$contact_class?>" id="contact_a_<?=$get_member[0]->member_id?>"
                        <?php
                            if ($contact_type == 1){?>onclick="return confirm_contact(<?=$get_member[0]->member_id?>)"<?php }
                            elseif ($contact_type == 0){?>onclick="return view_contact(<?=$get_member[0]->member_id?>)"<?php }?>>
                        <span id="contact_text">
                            <i class="fa fa-mobile"></i> <?=$contact_text ? $contact_text.'<b> ( '.substr($get_member[0]->mobile, 0, 4) .str_repeat("*", strlen($get_member[0]->mobile)-4).' )</b>' : $contact_text ?>
                        </span>
                    </a>
                </div>
            </div>
