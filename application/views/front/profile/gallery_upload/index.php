...
<div class="card-body">
    <form class="form-default col-12" id="gallery_upload_form" method="post" action="<?=base_url()?>home/gallery_upload/add" role="form" enctype="multipart/form-data">
        ...
        <div class="form-group has-feedback col-10 ml-auto mr-auto text-center mt-5">
            ...
            <a id="submit_gallery" class="btn btn-sm btn-base-1 btn-shadow gupload" onclick="return confirm_gallery_upload(<?=$this->session->userdata('member_id')?>)" style="color: white"><?php echo translate('upload')?></a>
        </div>
    </form>  
</div>
<script>
    ...
    function readURL_all(input) {
        ...
        var file_size = $('#image_main')[0].files[0].size;
        //alert(file_size);
        if(file_size>2097152) {
        $("#file_error1").html("File size should not be greater than 2MB");
        $('.gupload').addClass('d-none'); 
		//$(".demoInputBox").css("border-color","#FF0000");
        return false;
                  
	    }else{
         $('.gupload').removeClass('d-none');   
        }
    ...
    function confirm_gallery_upload(id) {
        ...
        if (isloggedin == "") {
            ...
            if (rem_photos <= 0) {
                ...
                $("#modal_body").html("<p class='text-center'><b><?php echo translate('add ')?>"+rem_photos+" <?php echo translate('Photo(s)_more_to_get_best_response')?></b><br><span style='color:#DC0330;font-size:11px'> <?php //echo translate('uploading_an_image_will_cost_1_from_your_remaining_gallery_uploads')?>**</span></p>");
