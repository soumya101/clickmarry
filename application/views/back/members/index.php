	...
	<div id="page-content">
		...
		<div class="panel">
			...
			<div class="panel-heading">
				...
				if($loginedType==1){
				?>
				<a href="<?= base_url()?>admin/exporttoexcel/<?= $member_type ?>" class="btn btn-success btn-sm" style="float: right;margin-right: 3%;margin-bottom: 8px;"><?= translate('Export_xls_/_csv');?></a>
				<?php }?>
			</div>
...
<div class="modal fade" id="otp_modal" role="dialog" tabindex="-1" aria-labelledby="otp_modal" aria-hidden="true">
    <div class="modal-dialog" style="width: 400px;">
        <div class="modal-content">
            <!--Modal header-->
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal"><i class="pci-cross pci-circle"></i></button>
                <h4 class="modal-title"><?php echo translate('confirm_otp_validate')?></h4>
            </div>
           	<!--Modal body-->
            <div class="modal-body">
            	<p><?php echo translate('are_you_sure_you_want_to validate OTP')?></p>
            	<div class="text-right">
            		<input type="hidden" id="member_id" name="member_id" value="">
            		<button data-dismiss="modal" class="btn btn-default btn-sm" type="button" id="modal_close"><?php echo translate('close')?></button>
                	<button class="btn btn-primary btn-sm" id="otp_status" value=""><?php echo translate('confirm')?></button>
            	</div>
            </div>
        </div>
    </div>
...
<script>
	...
	function otp_validate(id){
	    $("#otp_status").val(id);
	}

	$("#otp_status").click(function(){
    	$.ajax({
		    url: "<?=base_url()?>admin/member_otp_validated/"+$("#otp_status").val(),
		    success: function(response) {
				window.location.href = "<?=base_url()?>admin/members/<?=$parameter?>";
		    },
			fail: function (error) {
